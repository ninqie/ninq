    ...
    switch($action) {
      case 'install':
        try {
          ...
          $this->db->ExecuteQuery(self::SQL('insert content'), array('hello-world-again', 'post', 'Hello World Again', "[b]This[/b] is another demo post.\n\nThis is another row in this demo post.[img]http://www.student.bth.se/~anrb13/phpmvc/Kmom10/themes/core/logo_80x80.png[/img]", 'bbcode', $this->user['id']));
          ...
          $this->db->ExecuteQuery(self::SQL('insert content'), array('download', 'page', 'Download page', "This is a demo page, this could be your personal download-page.\n\nYou can download your own copy of ninq from https://github.com/ninqie/ninq", 'plain', $this->user['id']));
          $this->db->ExecuteQuery(self::SQL('insert content'), array('bbcode', 'page', 'Page with BBCode', "This is a demo page with some BBCode-formatting.\n\n[b]Text in bold[/b] and [i]text in italic[/i] and [url=http://dbwebb.se]a link to dbwebb.se[/url]. You can also include images using bbcode, such as the ninq logo: [img]http://www.student.bth.se/~anrb13/phpmvc/Kmom10/themes/core/logo_80x80.png[/img]", 'bbcode', $this->user['id']));
