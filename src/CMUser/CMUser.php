...
/**
  * Constructor
  */
      public function __construct($ninq=null) {
        parent::__construct($ninq);
 ...
 */
 	...
 	if (is_null($offset)) { 
 	$this->profile[] = $value; } else { $this->profile[$offset] = $value; 
 	 }
 	}
...
/**
  ...
  */
  	...
  	'create table user'       => "CREATE TABLE IF NOT EXISTS User (
        ...
        'create table group'      => "CREATE TABLE IF NOT EXISTS Groups (
       				     id INTEGER PRIMARY KEY, 
       				     acronym TEXT KEY, 
       				     name TEXT, 
       				     created DATETIME default (datetime('now')),
       	...
       	'create table user2group' => "CREATE TABLE IF NOT EXISTS User2Groups (
      				    	idUser INTEGER, 
      				    	idGroups INTEGER, 
      				    	created DATETIME default (datetime('now')), 
...
/*
  * New table in database.
 	 ...
 	 public function Init() {
 	   ...
 	   $this->db->ExecuteQuery(self::SQL('create table user2group'));
 	   $password = $this->CreatePassword('root');
 	   $this->db->ExecuteQuery(self::SQL('insert into user'), 
 	   	   array('root', 'The Admin', 'symphonaticz@hotmail.com', $password['algorithm'], $password['salt'], $password['password']));
 	   $idRootUser = $this->db->LastInsertId();
 	   $password = $this->CreatePassword('doe');
 	   $this->db->ExecuteQuery(self::SQL('insert into user'), array('doe', 'John/Jane Doe', 'doe@dbwebb.se', $password['algorithm'], $password['salt'], $password['password']));
 	   $idDoeUser = $this->db->LastInsertId();
 	   ...
 	   $this->session->AddMessage('notice', 'Successfully created the database tables (or left them untouched if they already existed).
 	   	   Oh and added admin. Login: root:root. Oh and simple user doe:doe');
 	   } catch(Exception $e) {
 ...
 */
 	 ...
 	 $password = array(
 	 	 'algorithm'=>($algorithm ? $algoritm : Cninq::Instance()->config['hashing_algorithm']),
 	 	 'salt'=>null
 	 	 );
